...
export default async function MatchesPage({
  ...
  // Helper to determine if a match is an internal FC BBFF match
  const isInternalMatch = (m: any) => {
    const home = (m.homeTeam?.name || "").toUpperCase();
    const away = (m.awayTeam?.name || "").toUpperCase();
    return home.includes("BBFF") && away.includes("BBFF");
  };
  ...
  if (filterType === "competition") {
    ...
    upcoming = upcoming.filter((m: any) => !isInternalMatch(m));
    completed = completed.filter((m: any) => !isInternalMatch(m));
  } else if (filterType === "internal") {
    upcoming = upcoming.filter((m: any) => isInternalMatch(m));
    completed = completed.filter((m: any) => isInternalMatch(m));
  }

  const renderBadge = (match: any) => {
    const internal = isInternalMatch(match);

    if (!internal) {
      return (
        <Badge className="bg-blue-500/10 text-blue-400 border border-blue-500/30 text-xs font-semibold">
          🌐 vs Outside Team
        </Badge>
      );
    }
    ...
    return (
      <Badge variant="outline" className="border-purple-500/30 text-purple-400 text-xs font-semibold">
        🏠 Internal Match
      </Badge>
    );
  };
    ...
    <div className="min-h-screen bg-neutral-950">
      <section className="relative overflow-hidden bg-gradient-to-br from-neutral-950 via-emerald-950/20 to-neutral-950 py-16 sm:py-20">
        ...
        <div className="relative mx-auto max-w-7xl px-4 text-center lg:px-8">
          <h1 className="text-4xl font-black text-white md:text-6xl tracking-tight">Matches & Fixtures</h1>
          <p className="mt-4 text-sm sm:text-base text-neutral-400 max-w-2xl mx-auto">
            Fixtures against external opponent clubs, official league tournaments, and internal squad matches
          </p>
        </div>
      </section>
      ...
      <section className="bg-neutral-950 py-12">
        <div className="mx-auto max-w-5xl px-4 lg:px-8">
          ...
          {upcoming.length > 0 && (
            <div className="mb-12">
              <h2 className="mb-6 flex items-center gap-2 text-2xl font-bold text-white">
                <Clock className="h-6 w-6 text-emerald-400" /> Upcoming Fixtures ({upcoming.length})
              </h2>
                  ...
                  >
                    <div className="flex flex-1 items-center gap-4">
                      <div className="text-right flex-1">
                        <p className="font-bold text-white text-base sm:text-lg">{match.homeTeam?.name}</p>
                      </div>
                      ...
                      <div className="text-left flex-1">
                        <p className="font-bold text-white text-base sm:text-lg">{match.awayTeam?.name}</p>
                      </div>
                    </div>
          ...
          {completed.length > 0 && (
            <div>
              <h2 className="mb-6 flex items-center gap-2 text-2xl font-bold text-white">
                <Trophy className="h-6 w-6 text-amber-400" /> Match Results ({completed.length})
              </h2>
              <div className="space-y-4">
          ...
          {upcoming.length === 0 && completed.length === 0 && (
            <div className="py-16 text-center text-neutral-500 border border-white/5 rounded-2xl bg-white/[0.02]">
              <p className="text-base font-semibold text-neutral-400">No matches found for this filter selection.</p>
            </div>
          )}
        </div>
      </section>
